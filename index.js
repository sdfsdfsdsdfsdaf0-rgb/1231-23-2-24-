...
const SUPPORT_GROUP_ID = -1003082469468;
...
bot.hears('🧩 Купить активацию', (ctx) => {
  ctx.reply(
    ...
    Markup.inlineKeyboard([
      [Markup.button.callback('SR Helper', 'select_srm')]
    ])
  );
});
...
bot.action('select_srm', async (ctx) => {
  ...
  ctx.editMessageText(
    ...
    Markup.inlineKeyboard([
    [Markup.button.url('Написать в поддержку', 'https://t.me/TstststysusBot?start=💬%20Поддержка')],
...
bot.command('hwid', async (ctx) => {
  if (!(await hasLevel(ctx.from.id, 3))) return ctx.reply('🚫 Нет доступа.');
  const args = ctx.message.text.split(' ').slice(1);
  if (!args[0]) return ctx.reply('❌ Укажите @username или ID');

  const data = await loadUsers();
  const userIdOrUsername = args[0];
  const id = resolveId(userIdOrUsername, data.users);
  if (!id) return ctx.reply('❌ Пользователь не найден');

  const user = data.users.find(u => u.id === id);
  ctx.reply(
    `🧾 HWID пользователя @${user.login || 'БезUsername'}:\n\n` +
    `<b>ID:</b> <code>${user.id}</code>\n` +
    `<b>HWID:</b> <code>${user.hwid || 'null'}</code>`,
    { parse_mode: 'HTML' }
  );
});

bot.command('rhwid', async (ctx) => {
  if (!(await hasLevel(ctx.from.id, 3))) return ctx.reply('🚫 Нет доступа.');
  const args = ctx.message.text.split(' ').slice(1);
  if (!args[0]) return ctx.reply('❌ Укажите @username или ID');
  ...
  await saveUsers(data);
  ctx.reply(`🔄 HWID пользователя @${user.login || 'БезUsername'} был сброшен.`);
});
...
bot.command('gsub', async (ctx) => {
  ...
  if (cheat?.days) {
    text += `📦 SR Helper: ${getRemainingDays(cheat)} дн осталось\n`;
    text += `Выдано: ${cheat.days} дн от ${cheat.issuedBy || 'неизвестно'} (${cheat.start})\n\n`;
  }
  if (lovla?.days) {
    text += `📦 buy2: ${getRemainingDays(lovla)} дн осталось\n`;
